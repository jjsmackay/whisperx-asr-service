...
from app import metrics as prom_metrics  # noqa: E402
from app.main import app  # noqa: E402


def _service_info_labels() -> dict:
    samples = list(prom_metrics.SERVICE_INFO.collect()[0].samples)
    return samples[0].labels if samples else {}


def test_lifespan_runs_and_health_responds():
    # Entering the TestClient context drives the ASGI lifespan startup, which
    # populates SERVICE_INFO as a side effect. Asserting on it proves the
    # lifespan body actually ran rather than the app merely importing.
    assert _service_info_labels() == {}

    with TestClient(app) as client:
        assert _service_info_labels().get("version")
