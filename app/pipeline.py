...
# Sweep interval has no implicit floor so that tests (and tight tuning) can
# drive the eviction loop on sub-30s cadences. Production deployments should
# still pick a sensible value (>=30s) via the env var.
MODEL_EVICTION_INTERVAL_SECONDS = max(
    1, int(os.getenv("MODEL_EVICTION_INTERVAL_SECONDS", "60"))
)
